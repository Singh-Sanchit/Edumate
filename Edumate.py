...
def enter_new_record(entryField, semester):
    ...
    if found == 0:
        if semester.get() == '--0--':
            ...
            # enter new record
            connection = pymysql.connect(host="localhost", user="root", passwd="040916030411", database="edumate") # database connection
...
def welcome_page(student_info):
    ...
    photo = PhotoImage(file="welcome_page.png") # opening left side image - Note: If image is in same folder then no need to mention the full path
